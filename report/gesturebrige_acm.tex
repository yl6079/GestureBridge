...
mode driven by fully offline Vosk~\cite{vosk} speech recognition on the C270's
...
the fixes, an ensemble of a fine-tuned MobileNetV3-Small~\cite{howard2019mobilenetv3} and a
63-dimensional MediaPipe-landmark~\cite{zhang2020mediapipehands} MLP achieves \textbf{82.9\,\%} on a
...
Kaggle ``ASL Alphabet''~\cite{kaggleasl} set with ${\approx}87$k images across 29
...
We insert a MediaPipe HandLandmarker~\cite{lugaresi2019mediapipe,zhang2020mediapipehands} (Tasks API,
...
The winner (\texttt{c\_u50\_d20}) was exported to FP32 TFLite~\cite{tflite} (3.7\,MB)
...
scale-normalized $\to$ 63-d).  A scikit-learn~\cite{sklearn} MLP with hidden layers
...
\newblock \emph{J. Mach. Learn. Res.}, vol. 12, pp. 2825--2830, 2011.

\bibitem{kornblith2019do}
S.~Kornblith, J.~Shlens, and Q.~V. Le,
\newblock ``Do better ImageNet models transfer better?''
\newblock in \emph{Proc. IEEE/CVF Conf. Computer Vision and Pattern Recognition
  (CVPR)}, 2019, pp. 2661--2671.
