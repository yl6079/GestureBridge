...
| 0:35 – 1:05 | **Read mode** screen-cap. Sign A, B, C, L, Y in front of camera. Each letter appears on screen; TTS audio plays. Show the confidence number | *"In read mode, MediaPipe finds the hand, crops it, and a fine-tuned MobileNetV3 predicts the letter. We added a landmark-MLP ensemble — when the two heads disagree we trust the geometric one, because ASL is mostly hand shape."* |
...
| 1:25 – 1:50 | **Trainer** mode. Random target letter; user signs it; system shows "✓" or "✗" | *"Trainer mode picks a target letter and gives true/false feedback while you practice."* |
